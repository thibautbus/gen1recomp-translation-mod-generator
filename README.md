...
`status_labels`) plus the empty 533-key `strings.lua` catalog; none are
...
strings` counts 383 keys from Gen1Recomp's 533-key catalog whose production
...
`5a48a61f2ed20aee80951aeb1e41b7ec084b350f`) scans production `src` callsites:
379 RBY-category keys plus four eligible mixed keys form the denominator; four
shared/link or UI-only keys require review, while 146 modern, network/link,
...
| `pipeline/engine.py` | Matches 533 engine strings using overrides, anchors, text, and placeholders. |
