...
from pipeline.shared.align import CORPUS_OVERRIDES_SCHEMA, align, apply_corpus_overrides
...
from pipeline.shared.join import (
...
from pipeline.shared.tokens import check_placeholders, encode
from pipeline.shared.validate import release_gate, validate
from pipeline.shared.worksheet import dump, load


class PipelineTests(unittest.TestCase):
    def test_cli_gen2_profile_defaults_to_pinned_and_rejects_versioned_names(self):
        with patch("pipeline.shared.cli.import_gs_rom") as import_gs:
    ...
    def test_cli_generate_defaults_engine_overrides_to_language_tree(self):
        for language, expected in (
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                output = Path(tmp) / "mod"
                with patch("pipeline.shared.cli.generate_mod") as generate:
                    self.assertEqual(cli_main([
    ...
    def test_cli_generate_warns_for_pokemon_profile(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            aligned.write_text(json.dumps([]), encoding="utf-8")
            output = io.StringIO()
            with patch("pipeline.shared.cli.generate_mod"), patch("sys.stderr", output):
                self.assertEqual(cli_main([
                    "generate", str(aligned), "-o", str(Path(tmp) / "mod"),
                    "--target-lang", "fr", "--font-profile", "pokemon",
                ]), 0)
...
class YellowCorpusTests(unittest.TestCase):
    ...
    def test_read_parallel_game_redblue_wrapper_keeps_rb_conventions(self):
        from pipeline.shared.corpus import read_parallel_game, read_parallel_redblue
        with tempfile.TemporaryDirectory() as tmp:
...
class YellowCliTests(unittest.TestCase):
    """CLI import/import-all/catalog accept a Yellow ROM, matching pipeline/shared/roms.py."""

    def test_import_accepts_yellow_version(self):
        with patch("pipeline.shared.cli.import_rom") as import_rom:
            self.assertEqual(cli_main([
    ...
    def test_catalog_includes_yellow_when_given(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.shared.cli.catalog_roms") as catalog_roms:
                self.assertEqual(cli_main([
    ...
    def test_import_all_includes_yellow_when_given(self):
        with patch("pipeline.shared.cli.import_all") as import_all:
            self.assertEqual(cli_main([
