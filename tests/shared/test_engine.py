...
from pipeline.shared.engine import (
...
from pipeline.shared.validate import release_gate
...
class EngineTests(unittest.TestCase):
    ...
    def test_gold_semantic_anchor_bares_a_named_ram_buffer(self):
        ...
        # (pipeline/gsc/engine.py:_corpus_records); this drives
    ...
    def test_real_gold_semantic_anchor_config_matches_the_bare_form(self):
        ...
        anchors_path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "semantic_anchors.json"
        output, report = match_engine_catalog(
    ...
    def test_gold_config_never_names_a_ram_or_num_buffer(self):
        ...
        gold_config_dir = Path(__file__).resolve().parents[2] / "config" / "gsc"
    ...
    def test_override_ships_unmapped_tokens_verbatim_without_corpus_to_engine(self):
        ...
        # absent from pipeline/shared/tokens.py's _CORPUS_EXPANSIONS) ship today --
