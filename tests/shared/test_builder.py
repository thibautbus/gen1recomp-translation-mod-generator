...
class BuilderTests(unittest.TestCase):
    ...
    def test_font_dependencies_use_private_cache_and_checked_in_pins(self):
        ...
        with tempfile.TemporaryDirectory() as directory, patch(
            ...
            "pipeline.shared.builder.fetch_archive", side_effect=lambda *args, **kwargs: args[2]
        ) as fetch_archive:
    ...
    def test_coverage_groups_rom_metrics_before_engine_metrics(self):
        ...
        # "All engine strings" total -- matches pipeline.shared.gui.coverage_lines'
        # ordering.
        with tempfile.TemporaryDirectory() as directory:
            report = Path(directory) / "coverage.json"
            report.write_text(json.dumps({
    ...
    def test_prerequisite_message_is_actionable(self):
        with (
            patch("pipeline.shared.builder.shutil.which", return_value=None),
            patch("pipeline.shared.builder.importlib.util.find_spec", return_value=None),
    ...
    def test_luajit_hint_uses_detected_linux_package_manager(self):
        with (
            patch("pipeline.shared.builder.platform.system", return_value="Linux"),
            patch(
                "pipeline.shared.builder.shutil.which",
    ...
    def test_pillow_hint_detects_bypassed_virtual_environment(self):
        with (
            patch.dict(
                "pipeline.shared.builder.os.environ",
                {"VIRTUAL_ENV": "/workspace/venv"},
                clear=True,
            ),
            patch("pipeline.shared.builder.sys.executable", "/usr/bin/python3"),
            patch("pipeline.shared.builder.platform.system", return_value="Linux"),
        ):
