...
class DependencyTests(unittest.TestCase):
    ...
    def test_tree_digest_order_is_portable_across_path_flavours(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.shared.dependencies.sorted", windows_sorted, create=True):
    ...
    def test_default_ssl_context_prefers_a_working_os_store(self):
        ...
        with patch("pipeline.shared.dependencies.ssl.create_default_context", return_value=working) as create_context:
    ...
    def test_default_ssl_context_falls_back_to_certifi_when_os_store_is_empty(self):
        ...
        with patch("pipeline.shared.dependencies.ssl.create_default_context", side_effect=fake_create_context) as create_context:
            context = _default_ssl_context()
    ...
    def test_default_ssl_context_falls_back_to_empty_os_context_without_certifi(self):
        ...
        with patch("pipeline.shared.dependencies.ssl.create_default_context", return_value=empty), \
    ...
    def test_windows_packaging_pins_full_luajit_clone(self):
        script = (Path(__file__).parents[2] / "packaging/build_windows_executable.ps1").read_text()
    ...
    def test_windows_release_downloads_artifact_after_checkout(self):
        workflow = (Path(__file__).parents[2] / ".github/workflows/standalone-executables.yml").read_text()
    ...
    def test_linux_packaging_script_is_pinned_and_cleans_runtime(self):
        script = (Path(__file__).parents[2] / "packaging/build_linux_executable.sh").read_text()
    ...
    def test_spec_bundles_the_tools_lua_scripts_and_gate_fixtures(self):
        ...
        # "cannot open ... tools\tools/gsc/extract.lua: No such file or
        # directory" -- resource_root()/"tools"/... (pipeline/shared/roms.py,
        # pipeline/gsc/mod.py) resolves to PyInstaller's extraction dir at
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            tools = root / "tools"
            (tools / "gsc" / "gate_fixtures" / "broken_gs").mkdir(parents=True)
            (tools / "gsc" / "extract.lua").write_text("-- fixture\n")
            (tools / "gsc" / "gate.lua").write_text("-- fixture\n")
            (tools / "gsc" / "gate_fixtures" / "broken_gs" / "main.lua").write_text("-- fixture\n")
            ...
            found = {
                (tools / "gsc" / "extract.lua"): str(Path("tools") / "gsc"),
                (tools / "gsc" / "gate.lua"): str(Path("tools") / "gsc"),
                (tools / "gsc" / "gate_fixtures" / "broken_gs" / "main.lua"): str(Path("tools") / "gsc" / "gate_fixtures" / "broken_gs"),
            }
    ...
    def test_archive_mkdtemp_failure_cleans_download(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            destination = root / "out"
            with patch("pipeline.shared.dependencies.tempfile.mkdtemp", side_effect=OSError("cannot create temp directory")):
    ...
    def test_checked_in_corpus_manifest_has_all_pins(self):
        config = tomllib.loads((Path(__file__).parents[2] / "config/pipeline.toml").read_text())
