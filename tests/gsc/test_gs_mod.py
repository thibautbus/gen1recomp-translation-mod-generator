...
from pipeline.shared.builder import BuildError, _which_luajit
from pipeline.gsc.join import GsJoinEntry, NO_MATCH, UNIQUE
from pipeline.gsc.mod import (
...
from pipeline.gsc.mod import _gs_ui_labels, _write_dialogue_gate_expectation, _write_gate_expectations
...
GATE_SCRIPT = ROOT / "tools" / "gsc" / "gate_package.lua"
DIALOGUE_GATE_SCRIPT = ROOT / "tools" / "gsc" / "gate_dialogue.lua"
REGISTRIES_GATE_SCRIPT = ROOT / "tools" / "gsc" / "gate_registries.lua"
RBY_FIXTURE = ROOT / "tools" / "gsc" / "gate_fixtures" / "rby_translation"
...
class GenerateGsModTests(unittest.TestCase):
    def test_manifest_declares_gold_and_silver_and_nothing_else(self):
        ...
        # addresses closely enough (see pipeline/gsc/mod.py's comment above
...
class GsReleaseGateFlowTests(unittest.TestCase):
    ...
    def test_all_gates_run_before_a_release_is_allowed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.gsc.mod._run") as run:
            ...
            self.assertEqual(commands, ["gate.lua", "gate_dialogue.lua", "gate_registries.lua"])
    ...
    def test_manifest_keeps_only_compact_engine_coverage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.gsc.mod._run"):
...
class GsPackageGateTests(unittest.TestCase):
    ...
    def test_side_by_side_coexistence(self):
        ...
        self.assertEqual(
            result.returncode, 0,
            f"tools/gsc/gate_package.lua failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}",
        )
...
class TextCatalogFromJoinTests(unittest.TestCase):
    ...
    def test_a_resolved_gold_pointer_is_also_aliased_onto_its_silver_pointer(self):
        ...
        # entry (see tools/gsc/spike_text_overlap.lua's measurement):
...
class BuildGsDialogueModTests(unittest.TestCase):
    ...
    def test_engine_matches_are_shipped_and_reported_separately(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "pipeline.gsc.mod.match_gs_engine_strings",
...
class GsDialogueGateTests(unittest.TestCase):
    """tools/gsc/gate_dialogue.lua: a resolved pointer's translation is
    ...
    def test_translation_is_selected_and_unresolved_pointer_falls_back(self):
        ...
        # same JSON expectation file tools/gsc/gate_registries.lua already uses
        ...
        self.assertEqual(
            result.returncode, 0,
            f"tools/gsc/gate_dialogue.lua failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}",
        )
...
class GsRegistriesGateTests(unittest.TestCase):
    """tools/gsc/gate_registries.lua: pokemon/moves/items/trainer-class
    ...
    def test_each_registry_lands_at_its_routed_target(self):
        ...
        self.assertEqual(
            result.returncode, 0,
            f"tools/gsc/gate_registries.lua failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}",
        )
        self.assertIn("all gs registries gate checks passed", result.stdout)
...
class BuildGsEngineSourceTests(unittest.TestCase):
    ...
    def test_engine_source_reaches_prepare_build_context(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("pipeline.shared.orchestration.prepare_build_context", side_effect=fake_prepare_build_context), \
                    patch("pipeline.gsc.mod.verify_gs_rom"), \
                    patch("pipeline.gsc.mod.verify_crystal_rom"):
