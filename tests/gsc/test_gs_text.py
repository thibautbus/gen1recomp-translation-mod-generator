...
from pipeline.gsc.text import GsTextRecord, parse_gs_text_catalog


class GoldTextCatalogTests(unittest.TestCase):
    def test_parses_pointer_and_escaped_text(self):
        with tempfile.TemporaryDirectory() as tmp:
            text_tsv = Path(tmp) / "gs_text.tsv"
            text_tsv.write_text(
                "55:4067\tLine one\\nLine two\\ttabbed\n"
                "10:0001\t{text_start}Hello\\\\World<DONE>\n",
                encoding="utf-8",
            )
            records = parse_gs_text_catalog(text_tsv)
            self.assertEqual(records, [
                GsTextRecord("10:0001", "{text_start}Hello\\World<DONE>"),
                GsTextRecord("55:4067", "Line one\nLine two\ttabbed"),
            ])
    ...
    def test_resolves_labels_from_the_labels_tsv(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "gs_text.tsv").write_text("21:6514\tNew Hall of Famer!\n55:4067\tOther\n", encoding="utf-8")
            (root / "gs_labels.tsv").write_text(
                "AnimateHallOfFame.String_NewHallOfFamer\t21:6514\n", encoding="utf-8",
            )
            records = parse_gs_text_catalog(root / "gs_text.tsv", root / "gs_labels.tsv")
    ...
    def test_labels_tsv_is_optional(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            records = parse_gs_text_catalog(text_tsv)
            self.assertEqual(records, [GsTextRecord("55:4067", "Hello")])
    ...
    def test_rejects_a_pointer_repeated_with_different_text(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "ambiguous pointer '55:4067'"):
                parse_gs_text_catalog(text_tsv)
    ...
    def test_tolerates_a_pointer_repeated_with_identical_text(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            records = parse_gs_text_catalog(text_tsv)
            self.assertEqual(records, [GsTextRecord("55:4067", "Same")])

    def test_rejects_a_label_naming_a_pointer_absent_from_the_text_catalog(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "gs_text.tsv").write_text("55:4067\tHello\n", encoding="utf-8")
            (root / "gs_labels.tsv").write_text("SomeLabel\t99:9999\n", encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "SomeLabel.*99:9999"):
                parse_gs_text_catalog(root / "gs_text.tsv", root / "gs_labels.tsv")

    def test_rejects_two_different_labels_for_the_same_pointer(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "gs_text.tsv").write_text("55:4067\tHello\n", encoding="utf-8")
            (root / "gs_labels.tsv").write_text("LabelA\t55:4067\nLabelB\t55:4067\n", encoding="utf-8")
