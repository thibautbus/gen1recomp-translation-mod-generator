...
from pipeline.gsc.engine import (
    engine_string_keys, is_reviewed_identity, load_gs_engine_fallbacks,
...
def load_engine_no_op_entries(language):
    report = json.loads(
        (Path(__file__).resolve().parents[2] / "config" / "gsc" / "engine_fallbacks.json").read_text(
            encoding="utf-8",
        ),
    )
...
class GoldEngineCatalogTests(unittest.TestCase):
    ...
    def test_load_gs_engine_scope_exclusions_rejects_malformed_input(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                ...
                '"version": 2, "source_revision": "2148291c7139769cc1a6debf183cd0666f47e144", '
    ...
    def test_matches_full_and_gen2_metrics_and_omits_empty_values(self):
        ...
        with (
            ...
            patch("pipeline.gsc.engine.load_manifest", return_value=manifest),
            patch(
                "pipeline.gsc.engine.verified_source",
                return_value=(Path(tmp) / "src", Path(tmp), "0" * 40),
            ),
            patch("pipeline.gsc.engine.iter_callsites", return_value=CALLSITES),
            patch("pipeline.gsc.engine.load_engine_overrides", return_value={}),
        ):
    ...
    def test_rejects_a_gold_override_for_an_unknown_engine_key(self):
        ...
        with (
            ...
            patch("pipeline.gsc.engine.iter_callsites", return_value=CALLSITES),
            patch("pipeline.gsc.engine.load_engine_overrides", return_value={"Stale": {"override": "X"}}),
        ):
    ...
    def test_rejects_a_fallback_entry_for_an_unknown_engine_key_on_the_upstream_profile(self):
        ...
        with (
            tempfile.TemporaryDirectory() as tmp,
            patch("pipeline.gsc.engine.load_manifest", return_value=manifest),
            patch("pipeline.gsc.engine.checkout_revision", return_value="local-unversioned"),
            patch("pipeline.gsc.engine.iter_callsites", return_value=CALLSITES),
            patch("pipeline.gsc.engine.load_engine_overrides", return_value={}),
            patch(
                "pipeline.gsc.engine.load_gs_engine_fallbacks",
