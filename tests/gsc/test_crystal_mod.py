...
from pipeline.gsc.crystal_mod import (
...
from pipeline.gsc.crystal_registries import (
...
from pipeline.gsc.crystal_strings import match_crystal_engine_strings
from pipeline.shared.engine_profile import PINNED_PROFILE
from pipeline.gsc.join import REVIEWED_QID, join_gs_pointers
from pipeline.gsc.text import GsTextRecord
...
class JoinCrystalDialogueTests(unittest.TestCase):
    ...
    def test_missing_language_corpus_still_applies_a_hand_composed_dialogue_override(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_corpus(root / "corpus", "fr", [("c.text.Hello", "Hello!", "Bonjour!")])
            with patch(
                "pipeline.gsc.crystal_mod.load_crystal_dialogue_overrides",
    ...
    def test_ambiguous_english_is_resolved_by_a_reviewed_decision(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.gsc.crystal_mod.load_crystal_pointer_decisions", return_value={"00:0001": "c.SiteA.HereYouGo"}):
                entries, stats = join_crystal_dialogue(root / "extracted", root / "corpus", "fr")
...
class CrystalFeatureCatalogTests(unittest.TestCase):
    ...
    def test_registry_catalog_absent_crystal_language_with_no_overrides_is_an_explicit_english_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("pipeline.gsc.crystal_registries.load_crystal_registry_overrides", return_value={}):
    ...
    def test_registry_catalog_absent_crystal_language_still_ships_hand_composed_overrides(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("pipeline.gsc.crystal_registries.CRYSTAL_ONLY_ITEMS", frozenset({"BLUE_CARD"})),
                patch("pipeline.gsc.crystal_registries.load_crystal_registry_overrides", return_value=overrides),
            ):
                catalogs, stats = crystal_registry_catalogs(extracted, root / "corpus", "ko")
    ...
    def test_absent_crystal_language_with_no_overrides_is_an_explicit_english_fallback(self):
        with (
            ...
            patch("pipeline.gsc.crystal_strings.load_gs_engine_scope_exclusions", return_value={"A", "B"}),
            patch("pipeline.gsc.crystal_strings.load_engine_overrides", return_value={}),
            patch("pipeline.gsc.crystal_strings._load_selectors", return_value={}),
        ):
    ...
    def test_scanner_overrides_complete_the_specialized_crystal_match(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("pipeline.gsc.crystal_strings.match_engine_catalog") as matcher,
                patch("pipeline.gsc.crystal_strings._load_selectors", return_value={}),
            ):
    ...
    def test_feature_metrics_keep_upstream_rom_text_out_of_the_54_entry_aggregate(self):
        ...
        with (
            patch("pipeline.gsc.crystal_mod.crystal_registry_catalogs", return_value=({}, named_stats)),
            patch("pipeline.gsc.crystal_mod.join_crystal_rom_text", return_value=({}, {"translated": 7, "total": 7})),
            patch("pipeline.gsc.crystal_mod.match_crystal_engine_strings", return_value=({}, {"translated": 48, "total": 48})),
        ):
    ...
    def test_pinned_profile_keeps_the_54_autonomous_crystal_catalogs(self):
        ...
        with (
            patch("pipeline.gsc.crystal_mod.crystal_registry_catalogs", return_value=(named, named_stats)),
            patch("pipeline.gsc.crystal_mod.match_crystal_engine_strings", return_value=(strings, string_stats)),
        ):
