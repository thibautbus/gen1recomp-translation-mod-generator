...
--
-- Usage: luajit tools/gsc/gate_dialogue.lua <gen1recomp_root> <mod_dir>
...
-- that narrowing entirely -- see tools/gsc/gate_registries.lua's own
...
if not (engineRoot and modDir and expectationPath) then
  io.stderr:write("usage: luajit tools/gsc/gate_dialogue.lua <gen1recomp_root> <mod_dir> "
    .. "<expectation_json_path>\n")
  os.exit(2)
end
...
-- already are in tools/gsc/gate_registries.lua (a language with no Crystal
...
-- root = modDir's parent, path = its own name (see tools/gsc/gate.lua's
...
-- Gold/Silver's own, see pipeline.gsc.crystal_mod's own docstring), merged into
...
-- under GameVersion=="crystal" (pipeline.gsc.mod._CRYSTAL_GUARD/
-- dialogue_crystal.lua). Proven the same way tools/gsc/gate_registries.lua
