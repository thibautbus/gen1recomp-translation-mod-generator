...
--
-- Runs the same stages as tools/gsc/extract.lua, in the same order, up
-- through "scripts" (the stage that produces the text table) -- copied
-- rather than importing tools/gsc/extract.lua, since that script is a
...
--
-- Usage: luajit tools/gsc/spike_text_overlap.lua <gen1recomp_root> <gold_rom> <silver_rom>
...
-- Same order as tools/gsc/extract.lua/RomExtractorGen2:run(), stopping once the
