...
--
-- Usage: luajit tools/gsc/gate_registries.lua <gen1recomp_root> <mod_dir>

local engineRoot, modDir = ...
if not engineRoot or engineRoot == "" or not modDir or modDir == "" then
  io.stderr:write("usage: luajit tools/gsc/gate_registries.lua <gen1recomp_root> <mod_dir>\n")
  os.exit(2)
end
...
if expectationPath and expectationPath ~= "" then
  ...
  -- engine too (pipeline.gsc.mod.GS_PINNED_REQUIRED_REGISTRIES requires both
  ...
  -- BuildError; see pipeline.gsc.mod._write_gate_expectations).
