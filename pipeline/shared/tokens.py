...
def known_literal_tokens() -> frozenset[str]:
    ...
    (pipeline/gsc/join.py:audit_join) check against this rather than the
...
def corpus_to_engine(text: str, *, bare_dynamic_tokens: bool = False) -> str:
    ...
    pipeline/shared/engine.py:match_engine_catalog also calls this for its
    automated matches, but an `rby/engine.json` or
    `rby/yellow_engine.json` entry is used VERBATIM
    (pipeline/shared/engine.py:~1221, `out[source] = value` with no
    ...
    "wStringBuffer2"/"wStringBuffer4" etc. Gold callers (pipeline/gsc/join.py,
    pipeline/gsc/index_join.py, pipeline/gsc/mod.py) must pass True so a corpus row that
