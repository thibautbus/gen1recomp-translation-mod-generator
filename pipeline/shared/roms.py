...
# extractor contract (tools/gsc/extract.lua). Unlike RBY, [rom.gold]/
...
# Complete output contract of tools/gsc/extract.lua.  Keeping the list next
...
def import_gs_rom(
    ...
    through to tools/gsc/extract.lua as an extra argument so it reads the
    ...
    script = resource_root() / "tools" / "gsc" / "extract.lua"
...
# Crystal shares the canonical fingerprint registry and the tools/gsc/extract.lua
# extractor contract with Gold/Silver, but it is not interchangeable with
# either edition (its own corpus join, see pipeline/gsc/crystal_mod.py) -- so
...
def import_crystal_rom(
    ...
    Mirrors import_gs_rom(): same tools/gsc/extract.lua script and the same
    GS_REQUIRED_TSV output contract (tools/gsc/extract.lua's edition branches cover
