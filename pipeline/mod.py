...
def _catalog(rows: list[Alignment], title: str, language: str = "fr", reflow_line_breaks: bool = False, font_path: str | Path | None = None, font_size: int | None = None, excluded_from_reflow: set[str] | None = None) -> str:
    ...
    for row in sorted(rows, key=lambda item: item.qid):
        if row.translation is not None:
            row_reflow = reflow_line_breaks and not is_excluded_qid(row.qid, excluded_from_reflow)
...
def generate_mod(items: Iterable[Alignment], destination: str | Path, mod_id: str = "translation-fr", language: str = "fr", modkit_worksheet: str | Path | None = None, report_path: str | Path | None = None, engine_catalog: str | Path | None = None, engine_overrides: str | Path | None = None, strict_engine: bool = False, semantic_anchors: str | Path | None = None, semantic_anchor_decisions: str | Path | None = None, target_name: str | None = None, literal_handlers: str | Path | None = None, target_description: str | None = None, engine_source: str | Path | None = None, engine_scope: str | Path | None = None, font_source: str | Path | None = None, font_profile: str = "fusion", yellow_dialogue: dict[str, str] | None = None, yellow_stats: dict | None = None, yellow_catalogs: dict[str, dict[str, str]] | None = None, yellow_engine_overrides: dict[str, str] | None = None, precomputed_join: tuple[dict, dict] | None = None, reflow_line_breaks: bool = False) -> Path:
    ...
    # Fails safe: the entire engine-literal "strings" catalog is never
    # reflowed (see the CATALOGS loop below) regardless of which engine
    # module a given string lives in, so nothing needs computing for it
    # here.  What remains is qid-driven dialogue/catalog content, where
    # these two signals flag the parts still hooked into engine code or a
    # scripted battle -- anything they miss simply keeps today's
    # ROM-original line breaks rather than risking a broken display.
    excluded_from_reflow: set[str] = set()
    if reflow_line_breaks and engine_source is not None:
        from .battle_scope import battle_adjacent_text_keys, dynamic_text_lookup_keys, gen1recomp_root, trainer_won_text_keys
        excluded_from_reflow |= battle_adjacent_text_keys(gen1recomp_root(engine_source) / "data" / "scripts")
        excluded_from_reflow |= dynamic_text_lookup_keys(engine_source)
        excluded_from_reflow |= trainer_won_text_keys(engine_source)
    ...
    for name in CATALOGS:
        # Never reflow the engine-literal "strings" catalog: any text with
        # its own callsite somewhere in the engine (whichever module owns
        # it) is out of scope for the optimization, full stop -- see the
        # module docstring on pipeline.battle_scope for why.
        catalog_reflow = reflow_line_breaks and name != "strings"
        if name == "strings" and engine_values:
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, catalog_reflow and key not in excluded_from_reflow, pacing_font_path, pacing_font_size))}," for key, value in sorted(values.items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, catalog_reflow and key not in excluded_from_reflow, pacing_font_path, pacing_font_size))}," for key, value in sorted(joined[name].items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
        else:
            grouped = [row for row in rows if catalog_for(row.qid) == name]
            body = _catalog(grouped, name, language, catalog_reflow, pacing_font_path, pacing_font_size, excluded_from_reflow)
    ...
    if literal_path.is_file():
        ...
        _, generated_handlers = generate_handlers(rows, recipes, runtime, reflow_line_breaks=reflow_line_breaks, font_path=pacing_font_path, font_size=pacing_font_size, excluded_from_reflow=excluded_from_reflow)
    elif runtime.exists():
        runtime.unlink()
