...
def generate_mod(items: Iterable[Alignment], destination: str | Path, mod_id: str = "translation-fr", language: str = "fr", modkit_worksheet: str | Path | None = None, report_path: str | Path | None = None, engine_catalog: str | Path | None = None, engine_overrides: str | Path | None = None, strict_engine: bool = False, semantic_anchors: str | Path | None = None, semantic_anchor_decisions: str | Path | None = None, target_name: str | None = None, literal_handlers: str | Path | None = None, target_description: str | None = None, engine_source: str | Path | None = None, engine_scope: str | Path | None = None, font_source: str | Path | None = None, font_profile: str = "fusion", yellow_dialogue: dict[str, str] | None = None, yellow_stats: dict | None = None, yellow_catalogs: dict[str, dict[str, str]] | None = None, yellow_engine_overrides: dict[str, str] | None = None, precomputed_join: tuple[dict, dict] | None = None, reflow_line_breaks: bool = False) -> Path:
    ...
    if reflow_line_breaks and engine_source is not None:
        from .battle_scope import (
            battle_adjacent_text_keys, battle_module_dynamic_keys,
            gen1recomp_root, is_battle_engine_path, trainer_won_text_keys,
        )
        try:
            from .engine_scope import iter_callsites
            for row in iter_callsites(engine_source):
                if is_battle_engine_path(str(row.get("path", ""))):
                    excluded_from_reflow.add(str(row.get("source", "")))
    ...
    for name in CATALOGS:
        if name == "strings" and engine_values:
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks and key not in excluded_from_reflow, pacing_font_path, pacing_font_size))}," for key, value in engine_values.items())
            lines.append("}")
            body = "\n".join(lines) + "\n"
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks and key not in excluded_from_reflow, pacing_font_path, pacing_font_size))}," for key, value in sorted(values.items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks and key not in excluded_from_reflow, pacing_font_path, pacing_font_size))}," for key, value in sorted(joined[name].items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
        else:
            grouped = [row for row in rows if catalog_for(row.qid) == name]
            body = _catalog(grouped, name, language, reflow_line_breaks, pacing_font_path, pacing_font_size, excluded_from_reflow)
