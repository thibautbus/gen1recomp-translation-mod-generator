...
def _catalog(rows: list[Alignment], title: str, language: str = "fr", reflow_line_breaks: bool = False, font_path: str | Path | None = None, font_size: int | None = None) -> str:
    ...
    for row in sorted(rows, key=lambda item: item.qid):
        if row.translation is not None:
            lines.append(f"  [{lua_string(row.qid)}] = {lua_string(_display(row.translation, reflow_line_breaks, font_path, font_size))},")
    lines.append("}")
    return "\n".join(lines) + "\n"
...
def generate_mod(items: Iterable[Alignment], destination: str | Path, mod_id: str = "translation-fr", language: str = "fr", modkit_worksheet: str | Path | None = None, report_path: str | Path | None = None, engine_catalog: str | Path | None = None, engine_overrides: str | Path | None = None, strict_engine: bool = False, semantic_anchors: str | Path | None = None, semantic_anchor_decisions: str | Path | None = None, target_name: str | None = None, literal_handlers: str | Path | None = None, target_description: str | None = None, engine_source: str | Path | None = None, engine_scope: str | Path | None = None, font_source: str | Path | None = None, font_profile: str = "fusion", yellow_dialogue: dict[str, str] | None = None, yellow_stats: dict | None = None, yellow_catalogs: dict[str, dict[str, str]] | None = None, yellow_engine_overrides: dict[str, str] | None = None, precomputed_join: tuple[dict, dict] | None = None, reflow_line_breaks: bool = False) -> Path:
    ...
    for name in CATALOGS:
        if name == "strings" and engine_values:
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks, pacing_font_path, pacing_font_size))}," for key, value in engine_values.items())
            lines.append("}")
            body = "\n".join(lines) + "\n"
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks, pacing_font_path, pacing_font_size))}," for key, value in sorted(values.items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
            ...
            lines.extend(f"  [{lua_string(key)}] = {lua_string(_display(value, reflow_line_breaks, pacing_font_path, pacing_font_size))}," for key, value in sorted(joined[name].items()))
            lines.append("}")
            body = "\n".join(lines) + "\n"
        else:
            grouped = [row for row in rows if catalog_for(row.qid) == name]
            body = _catalog(grouped, name, language, reflow_line_breaks, pacing_font_path, pacing_font_size)
    ...
    if literal_path.is_file():
        ...
        _, generated_handlers = generate_handlers(rows, recipes, runtime, reflow_line_breaks=reflow_line_breaks, font_path=pacing_font_path, font_size=pacing_font_size)
    elif runtime.exists():
        runtime.unlink()
