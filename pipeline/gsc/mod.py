...
from .localized_registries import (
...
from .text import parse_gs_text_catalog
...
_GS_UI_HANDLER_PATH = Path(__file__).resolve().parents[2] / "config" / "gsc" / "literal_handlers.json"
...
def _write_dialogue_gate_expectation(
    ...
    resolved/unresolved/leak shape tools/gsc/gate_registries.lua already
...
def run_gs_release_gates(
    ...
    *,
    ...
    tools = resource_root() / "tools" / "gsc"
    fixtures = tools / "gate_fixtures"
    for script in ("gate.lua", "gate_dialogue.lua", "gate_registries.lua"):
    ...
    _run([luajit, str(tools / "gate.lua"), str(gen1recomp), str(fixtures)], log_fn=log_fn)
    ...
    try:
        _run([luajit, str(tools / "gate_dialogue.lua"), str(gen1recomp), str(mod_dir),
    ...
    try:
        command = [luajit, str(tools / "gate_registries.lua"), str(gen1recomp), str(mod_dir), str(expectation_path)]
    ...
    validation = {
        ...
        "checks": [
            {
                "tool": "pipeline.gsc.join.audit_join",
                "version": project_version(),
                "command": "internal Gold join audit",
                "status": "passed",
            },
            {
                "tool": "tools/gsc/gate.lua",
                "version": engine_revision,
                "command": "luajit tools/gsc/gate.lua <gen1recomp> tools/gsc/gate_fixtures",
                "status": "passed",
            },
            {
                "tool": "tools/gsc/gate_dialogue.lua",
                "version": engine_revision,
                "command": "luajit tools/gsc/gate_dialogue.lua <gen1recomp> <mod> <expectation_json_path>",
                "status": "passed",
            },
            {
                "tool": "tools/gsc/gate_registries.lua",
                "version": engine_revision,
                "command": "luajit tools/gsc/gate_registries.lua <gen1recomp> <mod> <expectations>",
                "status": "passed",
            },
        ],
    }
...
def build_gs_dialogue_mod(
    ...
    collection: used here (not in pipeline/gsc/crystal_mod.py, unlike the dialogue join)
    ...
    if engine.supports_gen2_registries or engine.supports_gen2_content_registries:
        ...
        # pipeline/gsc/crystal_mod.py's joiners).
    ...
    # 95.8% divergent from Gold/Silver's, see pipeline/gsc/crystal_mod.py's docstring,
