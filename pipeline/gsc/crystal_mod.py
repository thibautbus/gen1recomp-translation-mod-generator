...
Crystal shares Gold/Silver's `tools/gsc/extract.lua` extractor contract (the
...
strategy (`pipeline.gsc.join.join_gs_pointers` matches by text, not by
...
Gold/Silver (see pipeline/gsc/mod.py's build_gs()/generate_gs_mod()), applied
at runtime only when GameVersion.get() == "crystal" -- the same pattern
pipeline/shared/mod.py's RBY build uses for Yellow's own dialogue_yellow.lua layer,
...
from .engine import _corpus_records
from .join import GsJoinEntry, join_gs_pointers, read_corpus_rows
from .text import GS_POINTER_RE, parse_gs_text_catalog, split_lines
from ..shared.engine_profile import UPSTREAM_PROFILE, normalize_engine_profile, profile_for
...
def load_crystal_pointer_decisions(path: str | Path | None = None) -> dict[str, str]:
    ...
    Mirrors pipeline.gsc.join.load_gs_pointer_decisions() exactly, except the
    ...
    """
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "crystal_pointer_decisions.json"
    path = Path(path)
    if not path.is_file():
        return {}
    try:
...
def load_crystal_dialogue_overrides(language: str, path: str | Path | None = None) -> dict[str, str]:
    ...
    if path is None:
        path = (
            Path(__file__).resolve().parents[2] / "overrides" / language / "gsc" / "crystal_dialogue.json"
        )
    path = Path(path)
    if not path.is_file():
        return {}
...
def join_crystal_dialogue(
    ...
    Returns entries and their raw join stats (see pipeline.gsc.join.join_gs_pointers).
...
def parse_rom_text_catalog(path: str | Path) -> dict[str, str]:
    """Parse ``label<TAB>escaped text`` emitted by ``tools/gsc/extract.lua``."""
    result: dict[str, str] = {}
...
def load_crystal_rom_text_anchors(path: str | Path | None = None) -> dict[str, str]:
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "crystal_rom_text_anchors.json"
