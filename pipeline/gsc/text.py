...
Parsed from tools/gsc/extract.lua's gs_text.tsv/gs_labels.tsv output,
not from a real data/generated/text.lua: this project's Gold import
deliberately neuters RomExtractorGen2:write/:save (see
tools/gsc/extract.lua's module docstring), so there is no such file on
disk to parse. gs_text.tsv IS the Gold equivalent of RBY's text.lua for
this pipeline's purposes -- see pipeline/rby/yellow.py:parse_text_catalog for
...
corpus<->pointer join primitives tools/gsc/measure_join.py measured this
backlog's go/no-go with, and pipeline/gsc/join.py's join reuses verbatim
...
# validator in the pipeline (pipeline/gsc/join.py, pipeline/gsc/crystal_mod.py)
...
def unescape(value: str) -> str:
    """Undo tools/gsc/extract.lua's TSV escaping (\\\\, \\n, \\r, \\t)."""
    out: list[str] = []
    index = 0
...
def split_lines(text: str) -> list[str]:
    ...
    fragments. tools/gsc/measure_join.py documents the measured cost of this
