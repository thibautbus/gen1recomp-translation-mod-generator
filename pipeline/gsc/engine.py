...
def load_gs_engine_fallbacks(
    ...
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "engine_fallbacks.json"
...
def load_gs_engine_scope_exclusions(path: str | Path | None = None) -> set[str]:
    ...
    """
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "engine_scope_exclusions.json"
    path = Path(path)
    if not path.is_file():
        return set()
    try:
...
def match_gs_engine_strings(
    ...
    # tools/gsc/gate_registries.lua's "does not leak into gold/silver" checks
