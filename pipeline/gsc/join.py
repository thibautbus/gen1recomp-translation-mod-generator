...
from ..shared.corpus import corpus_literals, corpus_target_text, localize_literals
from .text import GS_POINTER_RE, GsTextRecord, normalise, split_lines
from ..shared.tokens import DYNAMIC_TOKEN_RE, TOKEN_RE, check_placeholders, corpus_to_engine, known_literal_tokens
...
def load_gs_pointer_decisions(path: str | Path | None = None) -> dict[str, str]:
    ...
    """
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "pointer_decisions.json"
    path = Path(path)
    if not path.is_file():
        return {}
    try:
...
def load_gs_dialogue_overrides(
    ...
    Mirrors pipeline.gsc.crystal_mod.load_crystal_dialogue_overrides(): unlike
    ...
    if path is None:
        path = (
            Path(__file__).resolve().parents[2] / "overrides" / language / "gsc" / "dialogue.json"
        )
    path = Path(path)
    if not path.is_file():
        return {}
...
def load_gs_placeholder_decisions(
    ...
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "placeholder_decisions.json"
    path = Path(path)
    if not path.is_file():
        return {}
    try:
...
def load_gold_silver_pointer_aliases(path: str | Path | None = None) -> dict[str, str]:
    ...
    """
    if path is None:
        path = Path(__file__).resolve().parents[2] / "config" / "gsc" / "silver_pointer_aliases.json"
    path = Path(path)
    if not path.is_file():
        return {}
    try:
...
def read_corpus_rows(corpus_dir: str | Path, target_lang: str = "fr") -> list[tuple[str, str, str]]:
    ...
    literals = corpus_literals(qids, targets)
    return [
        (qid, en, localize_literals(corpus_target_text(target), literals))
        for qid, en, target in zip(qids, ens, targets)
    ]
    ...
    for entry in entries:
        ...
        for token in TOKEN_RE.findall(entry.translation):
            ...
            if re.fullmatch(r"\\x[0-9A-Fa-f]{2}", token):
                # Documented, deliberate debt (pipeline/shared/tokens.py): both RBY
...
def to_aligned_rows(entries: list[GsJoinEntry], target_lang: str = "fr") -> list[dict]:
    """Serialize entries into the flat aligned.json shape pipeline/shared/cli.py's
    ``validate``/``generate`` commands already read (see the loader at
    pipeline/shared/cli.py:129-140) -- so Gold's join flows through the existing
